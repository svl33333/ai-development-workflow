...
|---|---|---|---|---|
| RQ-01 | base・implementation・review revisionと許可metadata commit集合を分離し、inputごとのrevision/classとpresentation receiptを検証 | `review-contract.test.js` revision/class contract | 未許可commit・review head不一致・stale receipt | review bundle / preflight result |
| RQ-02 | input path、revision、artifact class、hash basis、Git object ID、path scope、expected change scope、work identityを固定 | `review-contract.test.js` scope and identity | scope外変更・digest/object ID不一致 | bundle digest |
...
| RQ-04 | 前回recordを再読込し、digest・round・全Critical/High findingのdisposition・fix revision・rangeを実体照合 | `review-contract.test.js` re-review lineage | 欠落record、finding省略、未知finding、未検証range | review record |
...
| RQ-06 | aggregate申告ではなく実review群と人間承認receiptからplan gateを導出し、最新qualifying APPROVEへ連続chainを要求 | `workflow.test.js` approved record set | `NEEDS_WORK`のみ、APPROVE×3後の最新NEEDS_WORK、digest不一致 | stage evidence |
| RQ-07 | named branch、durable exclusive lock、HEAD/branch/generation、recovery、cleanup、実serial fallbackを固定 | `worktree-manager.test.js` real Git fixture | parent共有、lock/branch衝突、dirty recovery、active task中serial起動 | worktree record |
...
- Clean checkout verification is an isolated copyでの `npm ci` → `npm test` → `npm run validate:fixtures` と、Node/npm/Git最低version実測。`node_modules` は成果物に含めず、検証完了後に削除する。
