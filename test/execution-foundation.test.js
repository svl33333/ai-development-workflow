...
test('scheduler executes independent units in parallel and rejects cycles', () => {
  const scheduler = new TaskScheduler(); scheduler.load({ approval_digest: 'approved', max_parallel_codex_tasks: 3, units: [unit('a'), unit('b'), unit('c', ['a'])] });
  ...
  assert.throws(() => new TaskScheduler().load({ approval_digest: 'approved', max_parallel_codex_tasks: 3, units: [unit('a', ['b']), unit('b', ['a'])] }), /dependency cycle/);
});

test('superseded generation rejects state-changing operations', () => {
  const scheduler = new TaskScheduler({ generation: 1 }); scheduler.load({ approval_digest: 'approved', max_parallel_codex_tasks: 3, units: [unit('a')] }); scheduler.activateSuccessor(2);
...
test('child execution fails closed without a real worktree or local review', async () => {
