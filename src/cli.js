...
export async function run(args) {
  ...
  if (!command || ['setup', 'onboard', 'status', 'validate', 'next', 'update', 'consume-bridge'].includes(command) === false) throw Object.assign(new Error('command must be setup, onboard, status, validate, next, update, or consume-bridge'), { code: 2 });
  ...
  if (command === 'consume-bridge') { const result = await consumeBridgeEvents(root, { ensure_orchestrator: async () => { const lifecycle = await ensureOrchestrator(store); await store.update((state) => ({ ...state, orchestrator_id: lifecycle.orchestrator_id, orchestrator_generation: lifecycle.generation, orchestrator_status: 'ACTIVE' })); } }); console.log(JSON.stringify(result, null, 2)); return 0; }
