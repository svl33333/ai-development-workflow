...
const PROTECTED_TRANSITION_FIELDS = ['stage','evidence'];
<<<<<<< HEAD
...
function advanceStage(state,nextStage,evidence,context={}) { validateState(state);const i=WORKFLOW_STAGES.indexOf(state.stage);if(WORKFLOW_STAGES.indexOf(nextStage)!==i+1)throw new Error(`INVALID_STAGE_TRANSITION: ${state.stage}->${nextStage}`);if(!evidence||STAGE_REQUIREMENTS[nextStage].some((k)=>typeof evidence[k]!=='string'||!evidence[k]))throw new Error(`STAGE_EVIDENCE_REQUIRED: ${nextStage}`);if(!context.workspaceRoot)throw new Error('STAGE_WORKSPACE_REQUIRED');const verified=validateStageEvidence(context.workspaceRoot,nextStage,evidence,{repositoryRoot:context.repositoryRoot??context.workspaceRoot});const c={...state,stage:nextStage,status:STAGE_STATUSES[nextStage][STAGE_STATUSES[nextStage].length-1],evidence:{...state.evidence,[nextStage]:verified},updated_at:new Date().toISOString()};if(!stateValidator(c))throw new Error(`STAGE_EVIDENCE_INVALID: ${stateValidator.errors.map((e)=>e.instancePath||e.keyword).join(',')}`);return c; }
module.exports={STAGES,APPROVAL_KINDS,STATES,WORKFLOW_STAGES,STAGE_REQUIREMENTS,STAGE_STATUSES,PROTECTED_TRANSITION_FIELDS,deriveQualifyingPlanReviewIteration,initialState,validateState,transition,advanceStage,canonicalJson};
=======
const PROTECTED_TRANSITION_FIELDS = ['stage', 'evidence'];
function validateState(state) {
  if (!state || state.schema_version !== '1.0.0') throw new Error('STATE_SCHEMA_VERSION');
  if (!STATES.includes(state.status)) throw new Error(`STATE_STATUS: ${state.status}`);
  if (!state.workflow_id || !state.target_revision) throw new Error('STATE_IDENTITY_REQUIRED');
  if (!WORKFLOW_STAGES.includes(state.stage)) throw new Error('STATE_STAGE_REQUIRED');
  if (!state.evidence || typeof state.evidence !== 'object') throw new Error('STATE_EVIDENCE_REQUIRED');
  if (!stateValidator(state)) throw new Error(`STATE_SCHEMA_INVALID: ${stateValidator.errors.map((error) => error.instancePath || error.keyword).join(',')}`);
  return true;
}
function transition(state, next, patch = {}) {
  validateState(state);
  if (PROTECTED_TRANSITION_FIELDS.some((key) => Object.prototype.hasOwnProperty.call(patch, key))) throw new Error('PROTECTED_STATE_PATCH');
  const allowed = { draft: ['preflighted', 'blocked'], preflighted: ['reviewing', 'blocked', 'auth_waiting', 'connection_waiting'], reviewing: ['approved', 'blocked', 'auth_waiting', 'connection_waiting', 'result_unknown'], approved: ['completed', 'blocked'], completed: [], blocked: ['draft', 'preflighted'], auth_waiting: ['preflighted', 'blocked'], connection_waiting: ['reviewing', 'blocked'], result_unknown: ['reviewing', 'blocked'] };
  if (!allowed[state.status].includes(next)) throw new Error(`INVALID_TRANSITION: ${state.status}->${next}`);
  const candidate = { ...state, ...patch, status: next, updated_at: new Date().toISOString() };
  if (!STAGE_STATUSES[candidate.stage].includes(next)) throw new Error(`STATUS_STAGE_MISMATCH: ${next}/${candidate.stage}`);
  if (next === 'preflighted' && candidate.preflight_passed !== true) throw new Error('PREFLIGHT_REQUIRED');
  if (next === 'approved' && (candidate.plan_approved !== true || candidate.approved_revision !== candidate.target_revision)) throw new Error('PLAN_APPROVAL_REQUIRED');
  return candidate;
}
function advanceStage(state, nextStage, evidence, context = {}) {
  validateState(state);
  ...
  if (nextIndex !== currentIndex + 1) throw new Error(`INVALID_STAGE_TRANSITION: ${state.stage}->${nextStage}`);
  if (!evidence || STAGE_REQUIREMENTS[nextStage].some((key) => typeof evidence[key] !== 'string' || evidence[key].length === 0)) throw new Error(`STAGE_EVIDENCE_REQUIRED: ${nextStage}`);
  if (!context.workspaceRoot) throw new Error('STAGE_WORKSPACE_REQUIRED');
  const verifiedEvidence = validateStageEvidence(context.workspaceRoot, nextStage, evidence, { repositoryRoot: context.repositoryRoot ?? context.workspaceRoot });
  const candidate = { ...state, stage: nextStage, status: STAGE_STATUSES[nextStage][STAGE_STATUSES[nextStage].length - 1], evidence: { ...state.evidence, [nextStage]: verifiedEvidence }, updated_at: new Date().toISOString() };
  if (!stateValidator(candidate)) throw new Error(`STAGE_EVIDENCE_INVALID: ${stateValidator.errors.map((error) => error.instancePath || error.keyword).join(',')}`);
  return candidate;
}
module.exports = { STATES, WORKFLOW_STAGES, STAGE_REQUIREMENTS, STAGE_STATUSES, PROTECTED_TRANSITION_FIELDS, validateState, transition, advanceStage, canonicalJson };
>>>>>>> 072fc7b (Close workflow execution bypasses)
