...
export class WorkflowOrchestrator {
  ...
  async next() {
    ...
    if (gateKind) {
      ...
      try { const approval = await this.loadApproval(gateKind); if (approval.valid !== true || approval.work_id !== current.work_id) throw new Error('invalid approval'); const boundKinds = { prototype_implementation: ['prototype_design'], production_spec: ['spec', 'production_spec'], production_plan: ['plan', 'production_plan'] }[gateKind]; if (boundKinds && !this.syntheticEvidence && !(await this.approvalMatchesCurrentPresentation(current, approval, boundKinds))) throw new Error('stale approval binding'); if (gateKind === 'prototype_implementation' && !this.syntheticEvidence && (current.qualifying_prototype_review_iteration < 3 || !current.prototype_model_confirmed || !current.prototype_review_conversation_id || !current.prototype_model_user_confirmed)) throw new Error('prototype review gate is incomplete'); if (gateKind === 'production_plan' && (!approval.manifest_digest || approval.manifest_digest !== current.execution_plan_digest)) throw new Error('stale execution-plan approval'); }
  ...
  async productionFixCompleted() {
    ...
    let fixExecution = null; if (this.executionEngine && fixPlan) fixExecution = await this.executionEngine.run(fixPlan, { baseRevision: state.current_revision ?? state.base_revision ?? 'HEAD', activeGeneration: state.orchestrator_generation, approvedDigest: state.execution_plan_digest, prompt: 'Apply only the blocking PR review findings' });
