...
export class WorkflowOrchestrator {
  ...
  async prototypeReviewCompleted({ findings = [], improved = false, modelConfirmed = false } = {}) { const state = await this.assertActiveGeneration(); const blocking = findings.filter((finding) => ['CRITICAL', 'HIGH', 'IMPORTANT'].includes(String(finding.severity ?? '').toUpperCase()) && !finding.resolved); const qualifying = improved && blocking.length === 0; const updated = await this.store.update((current) => ({ ...current, prototype_review_iteration: current.prototype_review_iteration + 1, qualifying_prototype_review_iteration: qualifying ? current.qualifying_prototype_review_iteration + 1 : 0, prototype_model_confirmed: current.prototype_model_confirmed || modelConfirmed, prototype_review_findings: findings, prototype_review_history: [...(current.prototype_review_history ?? []), { round: current.prototype_review_iteration + 1, findings, improved, blocking: blocking.length, model_confirmed: modelConfirmed }] })); return updated; }
