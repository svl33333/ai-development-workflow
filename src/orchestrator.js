...
export class WorkflowOrchestrator {
  ...
  async reserveExternalOperation(operation, targetRevision) {
    return this.store.fencedUpdate(this.generation, (current) => ({ ...current, active_external_operation: { operation, generation: this.generation, target_revision: targetRevision, status: 'reserved', reserved_at: new Date().toISOString() } }));
  }
  ...
  async chatgptStep(operation, stage, { conversationId = null } = {}) {
    ...
    if (!this.syntheticEvidence && operation === 'prototype_evaluation' && (!state.execution_plan || state.execution_plan.stage !== 'prototype' || !this.executionEngine || !state.prototype_execution_plan_digest)) throw new Error('live prototype evaluation requires the approved execution manifest, digest, and execution engine');
    ...
    await this.store.fencedUpdate(this.generation, (current) => ({ ...current, artifacts: [...(current.artifacts ?? []), { kind: operation, path: artifact, version: current.revision + 1 }], ...(response.execution_plan ? { execution_plan: response.execution_plan } : {}) }));
    return { ...response, conversation_id: actualConversationId, project_id: actual.id ?? actual.name };
  }
  async next() {
    const current = await this.assertActiveGeneration(); const target = nextStage(current.stage); if (!target) return { stage: current.stage, next_stage: null, mutation: false, stopped: true };
    ...
    if (gateKind) {
      ...
      try { const approval = await this.loadApproval(gateKind); if (approval.valid !== true || approval.work_id !== current.work_id) throw new Error('invalid approval'); const boundKinds = { prototype_implementation: ['prototype_design'], production_spec: ['spec', 'production_spec'], production_plan: ['plan', 'production_plan'] }[gateKind]; if (boundKinds && !this.syntheticEvidence && !(await this.approvalMatchesCurrentPresentation(current, approval, boundKinds))) throw new Error('stale approval binding'); if (gateKind === 'prototype_implementation' && !this.syntheticEvidence && (current.qualifying_prototype_review_iteration < 3 || !current.prototype_model_confirmed || !current.prototype_review_conversation_id || !current.prototype_model_user_confirmed || !approval.manifest_digest || approval.manifest_digest !== current.prototype_execution_plan_digest)) throw new Error('prototype review gate is incomplete'); if (gateKind === 'production_plan' && (!approval.manifest_digest || approval.manifest_digest !== current.execution_plan_digest)) throw new Error('stale execution-plan approval'); }
  ...
  async approve(kind, extra = {}) {
    ...
    if (!this.syntheticEvidence && kind === 'prototype_implementation' && (state.qualifying_prototype_review_iteration < 3 || !state.prototype_model_confirmed || !state.prototype_review_conversation_id || !state.prototype_actual_model || !state.prototype_model_user_confirmed)) throw new Error('prototype approval requires a bound three-round review and confirmed model');
    ...
    const approvalPath = path.join(this.productPath, '.ai-workflow', 'approvals', `${kind}.json`);
    const lockState = await this.state();
    const lockKey = `${lockState.project_id}-${lockState.work_id}`;
    await this.store.withLock(lockKey, async () => {
      const current = await this.state();
      if (current.orchestrator_generation !== this.generation || current.orchestrator_status !== 'ACTIVE') throw Object.assign(new Error('orchestrator generation is superseded'), { code: 3 });
  ...
  async prototypeDesignApproved() { const state = await this.assertActiveGeneration(); if (!this.syntheticEvidence && (state.qualifying_prototype_review_iteration < 3 || !state.prototype_model_confirmed)) throw new Error('prototype design requires three qualifying reviews and confirmed model'); if (!this.syntheticEvidence && !(await this.hasCurrentPresentation(state, ['prototype_design']))) throw new Error('prototype design must be presented before approval'); let manifestDigest = null; const plan = state.execution_plan; if (plan && Number.isInteger(plan.max_parallel_codex_tasks) && Array.isArray(plan.units)) { const unsigned = { ...plan }; delete unsigned.approval_digest; manifestDigest = crypto.createHash('sha256').update(JSON.stringify(unsigned)).digest('hex'); } if (!this.syntheticEvidence && (!plan || !manifestDigest || !this.executionEngine)) throw new Error('live prototype approval requires an execution manifest, canonical digest, and execution engine'); await this.approve('prototype_implementation', this.syntheticEvidence ? {} : { require_presentation_binding: true, manifest_digest: manifestDigest }); if (manifestDigest) await this.store.fencedUpdate(this.generation, (current) => ({ ...current, execution_plan: { ...current.execution_plan, approval_digest: manifestDigest }, prototype_execution_plan_digest: manifestDigest })); return this.setStage('prototype_implementation', 'running'); }
  async prototypeReviewCompleted({ findings = [], improved = false, conversationId = null, requiredModel = null, actualModel = null, userConfirmedModel = false } = {}) { const state = await this.assertActiveGeneration(); if (!this.syntheticEvidence && (!conversationId || !requiredModel || !actualModel || !userConfirmedModel)) throw new Error('prototype review requires conversation and model confirmation'); if (!this.syntheticEvidence && state.prototype_review_conversation_id && state.prototype_review_conversation_id !== conversationId) throw new Error('prototype review conversation must remain stable across rounds'); const blocking = findings.filter((finding) => ['CRITICAL', 'HIGH', 'IMPORTANT'].includes(String(finding.severity ?? '').toUpperCase()) && !finding.resolved); const qualifying = improved && blocking.length === 0; const updated = await this.store.fencedUpdate(this.generation, (current) => ({ ...current, prototype_review_iteration: current.prototype_review_iteration + 1, qualifying_prototype_review_iteration: qualifying ? current.qualifying_prototype_review_iteration + 1 : 0, prototype_model_confirmed: current.prototype_model_confirmed || userConfirmedModel, prototype_review_conversation_id: conversationId ?? current.prototype_review_conversation_id, prototype_required_model: requiredModel ?? current.prototype_required_model, prototype_actual_model: actualModel ?? current.prototype_actual_model, prototype_model_user_confirmed: current.prototype_model_user_confirmed || userConfirmedModel, prototype_review_findings: findings, prototype_review_history: [...(current.prototype_review_history ?? []), { round: current.prototype_review_iteration + 1, conversation_id: conversationId, findings, improved, blocking: blocking.length, required_model: requiredModel, actual_model: actualModel, user_confirmed_model: userConfirmedModel }] })); return updated; }
  async prototypeReviewRound({ conversationId = null, requiredModel = null, actualModel = null, userConfirmedModel = false } = {}) { const review = await this.chatgptStep('prototype_independent_review', 'prototype_review', { conversationId }); const refinement = await this.chatgptStep('prototype_refinement', 'prototype_improvement', { conversationId: review.conversation_id }); return this.prototypeReviewCompleted({ findings: review.findings ?? [], improved: refinement.improved === true, conversationId: review.conversation_id, requiredModel, actualModel, userConfirmedModel }); }
  async prototypeImplemented() { const state = await this.assertActiveGeneration(); if (this.executionEngine && state.execution_plan?.stage === 'prototype') { const approvedDigest = state.prototype_execution_plan_digest ?? state.execution_plan.approval_digest; if (!approvedDigest && !this.syntheticEvidence) throw new Error('approved prototype execution-plan digest is required'); const execution = await this.executionEngine.run(state.execution_plan, { baseRevision: state.current_revision ?? state.base_revision ?? 'HEAD', activeGeneration: state.orchestrator_generation, approvedDigest, assertActiveGeneration: () => this.assertActiveGeneration(), prompt: 'Execute the approved prototype units' }); await this.store.fencedUpdate(this.generation, (current) => ({ ...current, execution_result: execution, current_revision: execution.current_revision ?? current.current_revision })); } await this.chatgptStep('prototype_evaluation', 'prototype_evaluation'); return this.setStage('prototype_evaluation', 'waiting_for_chatgpt'); }
  ...
  async productionGrilled() {
    ...
    await this.store.fencedUpdate(this.generation, (current) => ({ ...current, artifacts: [...(current.artifacts ?? []), { kind: 'production_spec', path: artifact, version: current.revision + 1 }] }));
  ...
  async issueCreated() {
    ...
    await this.store.fencedUpdate(this.generation, (state) => ({ ...state, review_context: { ...state.review_context, planning_conversation_id: plan.conversation_id } }));
  ...
  async planReviewed() {
    ...
    const updated = await this.store.fencedUpdate(this.generation, (current) => {
  ...
  async planApproved() {
    ...
    let executionPlan = state.execution_plan ?? null; let manifestDigest = null;
    if (artifact) { try { const parsed = JSON.parse(await fs.readFile(path.resolve(this.productPath, artifact.path), 'utf8')); executionPlan = parsed.execution_plan ?? parsed; } catch { /* use the structured plan already returned by C2C */ } }
    if (executionPlan && Number.isInteger(executionPlan.max_parallel_codex_tasks) && Array.isArray(executionPlan.units)) { const unsigned = { ...executionPlan }; delete unsigned.approval_digest; manifestDigest = crypto.createHash('sha256').update(JSON.stringify(unsigned)).digest('hex'); executionPlan = { ...executionPlan, approval_digest: manifestDigest }; }
    if (!manifestDigest && !this.syntheticEvidence) throw new Error('a JSON execution-plan manifest is required before production approval');
    await this.approve('production_plan', manifestDigest ? { manifest_digest: manifestDigest } : {});
    if (executionPlan) await this.store.fencedUpdate(this.generation, (current) => ({ ...current, execution_plan: executionPlan, execution_plan_digest: manifestDigest }));
  ...
  async replacePlanReviewConversation() {
    ...
    return this.store.fencedUpdate(this.generation, (current) => ({ ...current, qualifying_plan_review_iteration: 0, review_context: { ...current.review_context, active_plan_review_conversation_id: replacement.conversationId, active_plan_review_project_id: project.id ?? project.name, active_plan_review_history_revision: current.revision + 1, active_plan_review_non_resumable_reason: null, replacement_history: [...current.review_context.replacement_history, { old_conversation_id: context.active_plan_review_conversation_id, new_conversation_id: replacement.conversationId, reason: context.active_plan_review_non_resumable_reason }] } }));
  }
  async implementationComplete() { const state = await this.assertActiveGeneration(); const planArtifact = [...(state.artifacts ?? [])].reverse().find((artifact) => artifact.kind === 'plan' || artifact.kind === 'production_plan'); let plan = state.execution_plan ?? null; if (!plan && planArtifact) { try { plan = JSON.parse(await fs.readFile(path.resolve(this.productPath, planArtifact.path), 'utf8')); } catch { plan = null; } } if (plan && this.executionEngine) { const approval = await this.loadApproval('production_plan'); if (!approval.manifest_digest || approval.manifest_digest !== plan.approval_digest || approval.manifest_digest !== state.execution_plan_digest) throw new Error('approved execution plan digest is stale or missing'); const execution = await this.executionEngine.run(plan, { baseRevision: state.current_revision ?? state.base_revision ?? 'HEAD', activeGeneration: state.orchestrator_generation, approvedDigest: approval.manifest_digest, assertActiveGeneration: () => this.assertActiveGeneration() }); await this.store.fencedUpdate(this.generation, (current) => ({ ...current, execution_result: execution, current_revision: execution.current_revision ?? current.current_revision })); } else if (plan && !this.syntheticEvidence) throw new Error('approved execution plan requires a configured execution engine'); return this.setStage('production_pr_draft', 'running'); }
  async prReviewed() {
    ...
    await this.store.fencedUpdate(this.generation, (current) => ({ ...current, unresolved_blocking_findings: blocking.length, pr_review_findings: review.findings ?? [] }));
  ...
  async productionFixCompleted() {
    ...
    let fixExecution = null; if (this.executionEngine && fixPlan) fixExecution = await this.executionEngine.run(fixPlan, { baseRevision: state.current_revision ?? state.base_revision ?? 'HEAD', activeGeneration: state.orchestrator_generation, approvedDigest: state.execution_plan_digest, assertActiveGeneration: () => this.assertActiveGeneration(), prompt: 'Apply only the blocking PR review findings' });
    ...
    await this.store.fencedUpdate(this.generation, (current) => ({ ...current, unresolved_blocking_findings: 0, review_iteration: (current.review_iteration ?? 0) + 1, current_revision: fixExecution?.current_revision ?? current.current_revision ?? current.base_revision, presentation_receipts: (current.presentation_receipts ?? []).map((receipt) => ({ ...receipt, approval_status: 'stale' })) }));
    return this.setStage('production_pr_draft', 'running');
  }
  async publishApproved() { await this.assertActiveGeneration(); const evidence = await this.evidenceProvider(await this.state()); await this.approve('pr_publish', evidence); return this.setStage('production_publish_waiting_approval', 'ready'); }
  ...
  async publish() { if (!this.github) throw Object.assign(new Error('live GitHub adapter is required'), { code: 4 }); const state = await this.assertActiveGeneration(); const evidence = await this.evidenceProvider(state); await this.requireApproval('pr_publish', { work_id: state.work_id, ...evidence }); if (!(await this.hasCurrentPresentation(state, ['pr_review', 'pr']))) throw Object.assign(new Error('PR review artifact must be presented before publishing'), { code: 4 }); await this.reserveExternalOperation('publish', evidence.target_revision); const created = await this.github.createPullRequest({ head: evidence.target_revision, issue_url: 'https://example.invalid/issues/1' }); await this.assertActiveGeneration(); const artifact = await writeArtifact(this.productPath, { projectId: state.project_id, stage: 'production_published', workId: state.work_id, artifactType: 'pr', version: state.revision + 1 }, `# Pull Request\n\n- number: ${created.number ?? evidence.pr_number ?? 'unknown'}\n- head: ${created.head ?? evidence.target_revision}\n`); await this.store.fencedUpdate(this.generation, (current) => ({ ...current, active_external_operation: null, artifacts: [...(current.artifacts ?? []), { kind: 'pr', path: artifact, version: current.revision + 1 }] })); return this.setStage('production_published', 'waiting_for_human'); }
  async mergeApproved() { await this.assertActiveGeneration(); const evidence = await this.evidenceProvider(await this.state()); await this.approve('pr_merge', evidence); return this.setStage('production_merge_waiting_approval', 'ready'); }
  async merge() { if (!this.github) throw Object.assign(new Error('live GitHub adapter is required'), { code: 4 }); const state = await this.assertActiveGeneration(); const evidence = await this.evidenceProvider(state); await this.requireApproval('pr_merge', { work_id: state.work_id, ...evidence }); if (!(await this.hasCurrentPresentation(state, ['pr_review', 'pr']))) throw Object.assign(new Error('PR review artifact must be presented before merging'), { code: 4 }); await this.reserveExternalOperation('merge', evidence.target_revision); await this.github.mergePullRequest({ number: evidence.pr_number, head: evidence.target_revision }); await this.assertActiveGeneration(); await this.store.fencedUpdate(this.generation, (current) => ({ ...current, active_external_operation: null })); return this.setStage('completed', 'completed', null); }
}
