...
export class TaskScheduler {
  ...
  load(plan, { approvedDigest = null } = {}) {
    ...
    for (const unit of plan.units ?? []) {
      ...
      ids.add(unit.unit_id); this.units.set(unit.unit_id, { ...unit, status: 'PENDING', max_attempts: unit.max_attempts ?? this.maxAttempts, attempts: 0, retry_of: null, successor_of: null, successor_spawned: false, run_id: null, generation: this.generation });
    }
  ...
  spawnSuccessor(unitId, generation = this.generation) { this.assertGeneration(generation); const predecessor = this.units.get(unitId); if (!predecessor || predecessor.status !== 'FAILED' || predecessor.successor_spawned) return null; const successorId = `${unitId}-successor`; if (this.units.has(successorId)) return null; predecessor.successor_spawned = true; predecessor.status = 'BLOCKED'; for (const dependent of this.units.values()) dependent.dependency_ids = dependent.dependency_ids.map((dependencyId) => dependencyId === unitId ? successorId : dependencyId); this.units.set(successorId, { ...predecessor, unit_id: successorId, status: 'PENDING', attempts: 0, retry_of: null, successor_of: unitId, successor_spawned: false, run_id: null, result: null, generation }); return { ...this.units.get(successorId) }; }
