...
export class TaskScheduler {
  ...
  spawnSuccessor(unitId, generation = this.generation) { this.assertGeneration(generation); const predecessor = this.units.get(unitId); if (!predecessor || predecessor.status !== 'FAILED' || predecessor.successor_spawned) return null; const successorId = `${unitId}-successor`; if (this.units.has(successorId)) return null; predecessor.successor_spawned = true; predecessor.status = 'BLOCKED'; this.units.set(successorId, { ...predecessor, unit_id: successorId, status: 'PENDING', attempts: 0, retry_of: null, successor_of: unitId, successor_spawned: false, run_id: null, result: null, generation }); return { ...this.units.get(successorId) }; }
