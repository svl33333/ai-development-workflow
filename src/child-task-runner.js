...
export class ChildTaskRunner {
  ...
  async run(unit, { baseRevision, prompt }) {
    ...
    const changedPaths = Array.isArray(execution.changed_paths) ? execution.changed_paths : (await this.worktreeManager.git?.changedPaths?.({ baseRevision, cwd: worktree.path }) ?? []);
    if (!this.allowSyntheticWorktree && !Array.isArray(changedPaths)) throw new Error(`child task did not return a changed-path manifest: ${unit.unit_id}`);
