...
function assertImplementationAllowed(state) { requireGate(state.status === 'approved', 'IMPLEMENTATION_STAGE_REQUIRED'); requireGate(state.plan_approved === true, 'PLAN_APPROVAL_REQUIRED'); requireGate(state.target_revision === state.approved_revision, 'APPROVED_REVISION_MISMATCH'); }
